...
def connect_and_do(action)
  ...
  begin
    timeout(@net_timeout) do
      Chef::Log.info("Calling #{action} on configuration #{@config_name} in Noah")
      @http = Net::HTTP.new(@noah_server, @noah_port)
      method("#{action}_configuration").call
    end
...
def create_configuration
  ...
  msg = {:format => @format, :body => @data}
  req = Net::HTTP::Put.new("/configurations/#{@config_name}")
...
def delete_configuration
  ...
  req = Net::HTTP::Delete.new("/configurations/#{@config_name}")
  handle_response(@http.request(req))  
end
...
def handle_response(res)
  ...
  case res.code
  when "200"
    Chef::Log.info("Configuration #{@config_name} #{data["action"]} successfully!")
