...
def connect_and_do(action)
  ...
  begin
    timeout(@net_timeout) do
      Chef::Log.info("Calling #{action} on service #{@service_name} for #{@host_name} in Noah")
      @http = Net::HTTP.new(@noah_server, @noah_port)
      method("#{action}_service").call
    end
...
def create_service
  ...
  msg = {:status => @status, :host_status => 'up'}
  req = Net::HTTP::Put.new("/services/#{@service_name}/#{@host_name}")
...
def delete_service
  ...
  req = Net::HTTP::Delete.new("/services/#{@service_name}/#{@host_name}")
  handle_response(@http.request(req))  
end
...
def handle_response(res)
  ...
  case res.code
  when "200"
    Chef::Log.info("Service #{@service_name} for #{@host_name} #{data["action"]} successfully!")
